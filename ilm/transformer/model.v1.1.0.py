...
class IntuinisticLanguageModel(nn.Module):
    ...
    def _coordinate_logits(self, batched_emb: torch.Tensor, start_positions: torch.Tensor):
        ...
        if target_roles.shape[0] != B:
            raise ValueError("start_positions must have one entry per batch item")
    ...
    def _unpack_training_batch(self, batch):
        if self.coordinate_token_embeddings or self.coordinate_lm_heads or self.word_row_transformer:
            if len(batch) != 3:
                raise ValueError(
