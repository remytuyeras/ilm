...
class IntuinisticLanguageModel(nn.Module):
    ...
    def generate(
        ...
        '''
        Generate new coordinate tokens from a context.

        The optional token_callback is the streaming hook used by the interactive
        UI. When provided, it receives each generated coordinate as soon as it is
        sampled, and show_progress=False can be used to avoid the tqdm bar.
        '''
        if syllable_num <= 0:
